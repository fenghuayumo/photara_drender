...
class MeshPreparationOptions:
    instant_vertex_count: int = 25_000
    instant_face_count: int = 25_000
    target_triangle_count: int = 120_000
    rosy: int = 4
    posy: int = 4
    ...
    atlas_gutter: float = 4.0
    atlas_max_stretch: float = 0.3
    atlas_quality: bool = False
...
def prepare_mesh_for_baking(
    ...
    *,
    ...
    if min(options.instant_vertex_count, options.instant_face_count, options.target_triangle_count) < 4:
        raise ValueError("mesh target counts must be greater than three")
    try:
        import pynanoinstantmeshes
        import trimesh
    except ImportError as error:
        raise ImportError("mesh preprocessing requires asdiff-render[baking]") from error
    ...
    remeshed_positions, remeshed_faces = pynanoinstantmeshes.remesh(
        source_positions,
        source_indices,
        vertex_count=options.instant_vertex_count,
        face_count=options.instant_face_count,
        rosy=options.rosy,
        posy=options.posy,
    ...
    process = subprocess.run(
        [str(executable), str(instant_path), str(manifold_path), str(options.target_triangle_count)],
        check=False,
        capture_output=True,
        text=True,
    )
    ...
    result = MeshPreparationResult(
        ...
        int(np.asarray(remeshed_positions).shape[0]),
        int(remeshed.faces.shape[0]),
