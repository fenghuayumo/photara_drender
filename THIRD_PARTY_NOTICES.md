...
The optional standalone `asdiff_mesh` / `asdiff_mesh_preprocessor` tools use CGAL Surface Mesh Simplification. That CGAL
package is available under GPL-3.0-or-later or a separate commercial CGAL license. It is disabled by default and is never
linked into the MIT `asdiff_render` library. When explicitly enabled, the standalone executable and `_asdiff_mesh`
...
into `asdiff_mesh` when `ASDIFF_ENABLE_INSTANT_MESHES=ON`.
