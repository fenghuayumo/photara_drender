...
The optional standalone `photara_mesh` / `photara_mesh_preprocessor` tools use CGAL Surface Mesh Simplification. That CGAL
package is available under GPL-3.0-or-later or a separate commercial CGAL license. It is disabled by default and is never
linked into the MIT `photara_drender` library. When explicitly enabled, the standalone executable and `_photara_mesh`
...
into `photara_mesh` when `PHOTARA_ENABLE_INSTANT_MESHES=ON`.
